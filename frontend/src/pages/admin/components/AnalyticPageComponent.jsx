...
const AnalyticPageComponent = ({ fetchOrdersForFirstDate, fetchOrdersForSecondDate, socketIOClient }) => {
  ...
  useEffect(() => {
    ...
    const handler = (newOrder) => {
      const orderDate = new Date(newOrder.createdAt).toLocaleString("en-US", { hour: "numeric", hour12: true, timeZone: "UTC" });
        ...
        if (today === new Date(firstDateToCompare).toDateString()) {
          setDataForFirstSet((prev) => {
            ...
            if (prev[length - 1].name === orderDate) {
              ...
              const lastElem = { name: orderDate, [firstDateToCompare]: prev[length - 1][firstDateToCompare] + newOrder.orderTotal.cartSubtotal };
          ...
          setDataForSecondSet((prev) => {
            ...
            if (prev[length - 1].name === orderDate) {
              ...
              const lastElem = { name: orderDate, [secondDateToCompare]: prev[length - 1][secondDateToCompare] + newOrder.orderTotal.cartSubtotal };
              return [...prev, lastElem];
            }
          });
        }
  ...
  const secondDateHandler = (e) => {
    if (e && e.target && e.target.value) {
      ...
      console.warn("Event target is undefined or does not have a value:", e);
    }
  };
  ...
  return (
    <Row className="m-5">
      ...
      <Col md={10}>
        <h1>
          Black Friday Cumulative Revenue {firstDateToCompare} VS {secondDateToCompare}
        </h1>
...
export default AnalyticPageComponent;
